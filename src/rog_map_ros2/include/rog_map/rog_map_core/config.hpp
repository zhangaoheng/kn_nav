...
#define RM_UNKNOWN_FLAG (-99999)
    ...
    private:
        ...
        bool LoadParam(string param_name, T &param_value, T default_value = T{}, bool required = false) {
            ...
            if (!nh_->has_parameter(param_name)) {
                nh_->declare_parameter<T>(param_name, default_value);
            }
            if (nh_->get_parameter(param_name, param_value)) {
            ...
            std::cout << param_value << std::endl;
            if (required) {
                throw std::invalid_argument(string("Required param " + param_name + " not found"));
            }
            return false;
        }
        ...
        bool LoadParam(string param_name, vector<T> &param_value, vector<T> default_value = vector<T>{},
            ...
            std::replace(param_name.begin(), param_name.end(), '/', '.');
            if (!nh_->has_parameter(param_name)) {
                nh_->declare_parameter<std::vector<T>>(param_name, default_value);
            }
            if (nh_->get_parameter(param_name, param_value)) {
                printf("\033[0;32m Load param %s success: \033[0;0m", param_name.c_str());
    ...
    public:
        ...
        Config(const rclcpp::Node::SharedPtr &nh,
            ...
            LoadParam(name_space + "/visualization/use_body_center", use_body_center_visualization, true);
...
//    RCLCPP_ERROR(nh_->get_logger(), "The inflation resolution is set.");
...
#define logit(x) (log((x) / (1 - (x))))
        ...
        bool use_body_center_visualization{true};
