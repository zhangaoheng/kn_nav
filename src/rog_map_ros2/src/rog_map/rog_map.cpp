...
ROGMap::ROGMap(const rclcpp::Node::SharedPtr& node) : node_(node) {
    ...
    vm_.vizcfg.use_body_center = cfg_.use_body_center_visualization;
    ...
    if (cfg_.visualization_en) {
        ...
        if (cfg_.viz_time_rate > 0) {
            vm_.viz_timer = node_->create_wall_timer(
    ...
    if (cfg_.ros_callback_en) {
        ...
        rc_.update_timer = node_->create_wall_timer(1ms, std::bind(&ROGMap::updateCallback, this));
    }
...
void ROGMap::vizCallback() {
    ...
    for (auto& marker : vm_.mkr_arr.markers) {
