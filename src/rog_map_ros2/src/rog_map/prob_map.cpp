...
GridType ProbMap::getInflatedGridType(const Vec3f& pos) const {
    return getInfGridType(pos);
}

bool ProbMap::insideLocalMapPublic(const Vec3f& pos) const {
    return insideLocalMap(pos);
}

bool ProbMap::isMapReady() const {
    return !map_empty_;
}

void ProbMap::getLocalMapBounds(Vec3f& box_min, Vec3f& box_max) const {
