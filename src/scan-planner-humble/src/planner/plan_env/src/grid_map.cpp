...
void GridMap::initMap(const rclcpp::Node::SharedPtr& node)
{
  ...
  scan_planner_ros2::getParam(node_, "grid_map/resolution", mp_.resolution_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/sliding_map_size_x", x_size, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/sliding_map_size_y", y_size, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/sliding_map_size_z", z_size, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/local_update_range_x", mp_.local_update_range_(0), x_size/2.0);
  scan_planner_ros2::getParam(node_, "grid_map/local_update_range_y", mp_.local_update_range_(1), y_size/2.0);
  scan_planner_ros2::getParam(node_, "grid_map/local_update_range_z", mp_.local_update_range_(2), z_size/2.0);
  
  scan_planner_ros2::getParam(node_, "grid_map/obstacles_inflation_z_up", mp_.obstacles_inflation_z_up, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/obstacles_inflation_z_down", mp_.obstacles_inflation_z_down, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/double_cylinder_radius", mp_.double_cylinder_radius_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/double_cylinder_offset", mp_.double_cylinder_offset_, 0.0);
  scan_planner_ros2::getParam(node_, "grid_map/map_sliding_en", mp_.map_sliding_en_, true);
  scan_planner_ros2::getParam(node_, "grid_map/map_sliding_thresh", mp_.map_sliding_thresh_, mp_.resolution_);

  scan_planner_ros2::getParam(node_, "grid_map/fx", mp_.fx_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/fy", mp_.fy_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/cx", mp_.cx_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/cy", mp_.cy_, -1.0);

  scan_planner_ros2::getParam(node_, "grid_map/depth_filter_maxdist", mp_.depth_filter_maxdist_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/depth_filter_mindist", mp_.depth_filter_mindist_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/depth_filter_margin", mp_.depth_filter_margin_, -1);
  scan_planner_ros2::getParam(node_, "grid_map/k_depth_scaling_factor", mp_.k_depth_scaling_factor_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/skip_pixel", mp_.skip_pixel_, -1);

  scan_planner_ros2::getParam(node_, "grid_map/p_hit", mp_.p_hit_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/p_miss", mp_.p_miss_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/p_min", mp_.p_min_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/p_max", mp_.p_max_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/p_occ", mp_.p_occ_, -1.0);
  scan_planner_ros2::getParam(node_, "grid_map/max_ray_length", mp_.max_ray_length_, -0.1);

  scan_planner_ros2::getParam(node_, "grid_map/vis_height", mp_.vis_height_, 0.3);
  scan_planner_ros2::getParam(node_, "grid_map/show_occ_time", mp_.show_occ_time_, false);

  scan_planner_ros2::getParam(node_, "grid_map/frame_id", mp_.frame_id_, string("world"));
  scan_planner_ros2::getParam(node_, "grid_map/sliding_map_frame_id", mp_.sliding_map_frame_id_, string("sliding_map"));
  scan_planner_ros2::getParam(node_, "grid_map/ground_height", mp_.ground_height_, 0.0);

  scan_planner_ros2::getParam(node_, "grid_map/sensor_type", mp_.sensor_type_, string("lidar"));
  scan_planner_ros2::getParam(node_, "grid_map/cloud_is_world", mp_.cloud_is_world_, true);
  scan_planner_ros2::getParam(node_, "grid_map/need_extrinsic", mp_.need_extrinsic_, true);
  ...
  /* init callback */
  tf_broadcaster_ = std::make_shared<scan_planner_tf2::TransformBroadcaster>(node_);

  if (mp_.sensor_type_ == "depth")
  {
    const auto qos = rclcpp::SensorDataQoS().get_rmw_qos_profile();
    depth_sub_.reset(new message_filters::Subscriber<sensor_msgs::msg::Image>(node_, "/grid_map/depth", qos));
    depth_pose_sub_.reset(
        new message_filters::Subscriber<nav_msgs::msg::Odometry>(node_, "/grid_map/sensor_pose", qos));
  ...
  else if (mp_.sensor_type_ == "lidar")
  {
    lidar_pose_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>(
        "/grid_map/sensor_pose", rclcpp::SensorDataQoS(),
  ...
  sliding_map_frame_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>(
      "/grid_map/body_pose", rclcpp::SensorDataQoS(),
...
void GridMap::updateOccupancyCallback()
{
  ...
  // cout << setprecision(7);
  // cout << "t2=" << (t2-t1).seconds() << " t3=" << (t3-t2).seconds() << " t4=" << (t4-t3).seconds() << endl;;
                                ...
                                const nav_msgs::msg::Odometry::ConstSharedPtr& pose)
{
  ...
  md_.ray_q_ = ray_q;
  md_.use_cloud_update_ = false;
  md_.last_observation_ns_ = node_->now().nanoseconds();
  updateSlidingMap(md_.ray_pos_);
...
bool GridMap::hasMapObservation() {
  return mp_.sensor_type_ == "lidar" ? md_.has_cloud_ : md_.has_first_depth_;
}

bool GridMap::mapFresh(double timeout_sec) {
  if (!hasMapObservation() || md_.last_observation_ns_ <= 0 || timeout_sec <= 0.0)
    return false;
  return (node_->now().nanoseconds() - md_.last_observation_ns_) <=
         static_cast<int64_t>(timeout_sec * 1e9);
}
