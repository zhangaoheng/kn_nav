...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    if (occ)
    {
        // disabled log(rclcpp::get_logger("path_searching"), "Start point is insdide an obstacle.");
        do
        {
    ...
    if (occ)
    {
        // disabled log(rclcpp::get_logger("path_searching"), "End point is insdide an obstacle.");
        do
        {
...
ASTAR_RET AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    while (!openSet_.empty())
    {
        ...
        {
            // time debug time_2 = rclcpp::Clock().now();
            // printf("\033[34mA star iter:%d, time:%.3f\033[0m\n",num_iter, (time_2 - time_1).seconds()*1000);
            // if((time_2 - time_1).seconds() > 0.1)
            //     RCLCPP_WARN(rclcpp::get_logger("path_searching"), "Time consume in A star path finding is %f", (time_2 - time_1).seconds() );
