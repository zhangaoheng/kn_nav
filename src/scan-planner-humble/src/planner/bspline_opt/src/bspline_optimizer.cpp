...
namespace scan_planner
{
  ...
  {
    scan_planner_ros2::getParam(node, "optimization/lambda_smooth", lambda1_, -1.0);
    scan_planner_ros2::getParam(node, "optimization/lambda_collision", lambda2_, -1.0);
    scan_planner_ros2::getParam(node, "optimization/lambda_feasibility", lambda3_, -1.0);
    scan_planner_ros2::getParam(node, "optimization/lambda_fitness", lambda4_, -1.0);

    scan_planner_ros2::getParam(node, "optimization/dist0", dist0_, -1.0);
    scan_planner_ros2::getParam(node, "optimization/max_vel", max_vel_, -1.0);
    scan_planner_ros2::getParam(node, "optimization/max_acc", max_acc_, -1.0);

    scan_planner_ros2::getParam(node, "optimization/order", order_, 3);
  }
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
      ...
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        if (flag_got_start && flag_got_end)
        {
          ...
          if (in_id >= 0 && out_id >= in_id && out_id < init_points.cols())
            ...
            RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "Skip invalid collision segment: in_id=%d, out_id=%d.", in_id, out_id);
        }
      }
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (ret == ASTAR_RET::SUCCESS)
      {
        ...
        if (path.size() < 2)
        {
          RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "a star path has less than 2 points, force return!");
          return a_star_paths;
        }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      {
        RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "Skip invalid A-star path while assigning control point directions.");
        continue;
      }
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (ret == ASTAR_RET::SUCCESS)
        {
          ...
          if (path.size() < 2)
          {
            RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "A-star path has less than 2 points, drop this collision segment.");
        ...
        else if (ret == ASTAR_RET::SEARCH_ERR && i + 1 < segment_ids.size())
        {
          ...
          --i;
          RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "A-star failed on one collision segment, merge it with the next segment.");
        }
        else
        {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        if (i >= a_star_paths.size() || a_star_paths[i].size() < 2)
        {
          RCLCPP_WARN(rclcpp::get_logger("bspline_opt"), "Skip invalid A-star path while assigning rebound directions.");
          continue;
        }
