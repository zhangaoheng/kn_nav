# 文件：planner_wrapper.py
# 用途：PCT 规划器对外 Python 封装——加载离线 tomogram（pickle），初始化 C++
#       底层规划器（lib 中的 ele_planner / a_star / traj_opt），对外提供
#       plan(start, end) 三维路径规划接口。
# 结构：TomogramPlanner —— 核心封装类。关键方法：loadTomogram（加载地图）、
#       initPlanner（构建 A*/优化器地图，含 gateway 与 clearance 代价）、
#       plan（A* 搜索 + GPMP 轨迹优化）。
...
class TomogramPlanner(object):
    ...
    # 代价，将各层展平后传给 C++ 的 OfflineElePlanner.init_map（兼容新旧接口）。
    ...
    # 起终点极近时直接返回单点/两点路径；A* 失败或路径为空返回 None。
