...
NumPy compatibility
-------------------
The pickle embeds the generating NumPy version's module references. NumPy 2.x
pickles arrays through ``numpy._core``, which does not exist in NumPy < 2
(the A2 runtime uses NumPy 1.21.5), so such files fail to load on the robot
with ``ModuleNotFoundError: No module named 'numpy._core'``.

Always regenerate the pickle in an environment matching the target runtime.
For A2 (NumPy 1.21.5, Python 3.10)::

    python3.10 -m venv .venv-np121
    .venv-np121/bin/pip install numpy==1.21.5 open3d
    .venv-np121/bin/python pcd_to_tomogram.py --pcd /path/to/map.pcd ...

The script prints a warning when NumPy >= 2 is detected, because the output
would not load in a NumPy 1.x environment.
...
# ============================================================
# 文件：pcd_to_tomogram.py —— PCD 地图转 PCT 规划器 tomogram pickle（CPU 版）。
# 用途：与 pcd_to_pickle.py 相同（详见文件顶部 docstring）：把点云地图转成
#       PCT 全局规划器所需的 5 层体素栅格 pickle；额外内置 NumPy 版本
#       兼容性警告，避免生成物在 A2 机器人（NumPy 1.21.5 / Python 3.10）
#       上因 numpy._core 缺失而无法加载。
# 结构：read_points 读点云 -> build_layers 体素化 -> traversability /
#       inflation / simplify 计算与精简 -> build_tomogram 组装 ->
#       main 落盘并自校验。
# 用法：python3 pcd_to_tomogram.py --pcd /path/to/map.pcd [--resolution 0.05]
# ============================================================
import argparse
import os
import pickle
...
# NumPy >= 2 时打印警告：其 pickle 引用 numpy._core，无法在 NumPy 1.x
# （A2 为 1.21.5）环境加载，提示用匹配版本重新生成。
def warn_numpy_compatibility():
    """Warn when the generating NumPy cannot produce A2-compatible pickles."""
    if int(np.__version__.split(".")[0]) < 2:
        return
    print(
        "\n[WARNING] NumPy >= 2 detected (current: "
        + np.__version__
        + "). Pickles generated now embed numpy._core references and will "
        "NOT load in NumPy 1.x environments (A2 uses NumPy 1.21.5).\n"
        "Regenerate under NumPy 1.21.5, e.g.:\n"
        "  python3.10 -m venv .venv-np121\n"
        "  .venv-np121/bin/pip install numpy==1.21.5 open3d\n"
        "  .venv-np121/bin/python pcd_to_tomogram.py --pcd ... --output ...\n",
        flush=True,
    )
...
# 体素化点睛（CUDA 内核的 numpy 复刻）：点投到栅格单元后按切片归属，
# k<=s 写高程层取 max，k>s 写净空层取 min；先按单元号稳定排序再用
# reduceat 聚合，避免逐单元重排序。
...
# 自由间隙的偏差线性加价；再按梯度分 可站立/可跨越/过陡 三档叠加代价，
# 稀疏可跨越区（窗口内可站立单元不足）直接判为障碍。
...
# 入口：先打印 NumPy 兼容性警告，校验 PCD、按需百分位裁剪离群点、
# 构建 tomogram 并落盘 pickle，最后重新加载打印 schema 自校验。
def main(argv=None):
    args = parse_args(argv)
    if not os.path.isfile(args.pcd):
        sys.exit(f"PCD file does not exist: {args.pcd}")

    warn_numpy_compatibility()
