...
- 分析：正常跟踪走廊原来会按当前偏差自动放宽，例如起点偏差 `0.42 m` 时允许走廊增至 `0.52 m`，形成“切角误差越大、下一条轨迹自由度越大”的正反馈。异常段经过 `LOCAL_AVOIDANCE` 后起点偏差达到 `0.80～0.87 m`，但走廊上限固定为 `0.60 m`，整轨安全检查又从 `t=0` 开始，导致所有回归轨迹在起点被拒绝。控制器在旧轨迹到期但未到终点时仍持续追踪旧终点，加上 Go2-W 不能执行规划器给出的横向速度，因此出现继续移动、逐渐降速并停住。同期 FAST-LIO 始终 `NORMAL/valid=1`，排除定位跳变为本轮主因。
- 修改：正常无障碍跟踪固定使用 `0.15 m` 严格走廊，不再根据现有误差整体放宽；机器人位于走廊外时进入 `RECOVER_GLOBAL`，起点许可宽度取当前偏差加一个地图分辨率，并在默认 `1.5 m` 轨迹弧长内线性收紧到 `0.15 m`，避障时同样只允许起点超限并逐步收紧到 `0.60 m`，从而消除 `t=0` 回归死锁且避免整条轨迹长期变宽。控制器新增基于航向误差的连续转弯降速和旧轨迹终点等待超时停车；Go2-W 的控制器横向速度限制同步为 `0`，航向停车阈值由 `0.8 rad` 收紧为 `0.35 rad`。A2/B2/Go2/local 仅同步新能力，保留原有 `0.8 rad` 航向阈值及各自速度、横移能力、外参和碰撞体参数。统一配置、拆分配置和 SCAN 默认配置已同步新参数，并增加启动参数日志以确认部署版本。
- 验证：全部相关 YAML 解析、`git diff --check`、Go2-W 路径安全契约及全机型参数保留契约通过；`scan_planner`、`pct_scan_navigation` 使用 `--symlink-install -DBUILD_TESTING=OFF` 编译通过。完整导航契约为 `16/18`，两项失败仍是仓库既有的全局重定位开关旧断言和 coordinator 参数旧期望，与本次修改无关。
- 遗留事项：同步到实机后先低速测试直角弯和连续弯，启动日志应出现 `Corridor policy` 以及 Go2-W 的 `heading_stop=0.35、max_velocity=(0.40,0.00,1.00)、end_timeout=0.50`；主动把机器人放到全局路径外 `0.6～0.9 m`，应看到 `RECOVER_GLOBAL` 且 `corridor_start` 随当前位置变化、`corridor_target=0.15`，机器人应逐步汇回中心线。若频繁原地转向，可在不放宽走廊的前提下小幅增加 `heading_error_threshold`，不应恢复旧的动态整体放宽逻辑。
